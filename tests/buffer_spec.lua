...
describe("async taskfile buffers", function()
    ...
    it("opens, updates, and reopens the generated view without readonly or file-change warnings", function()
        ...
        wait_for(3)
        requests[3].cb("# Today\nUpdated task\n")
        assert.are.equal(task_buf, vim.api.nvim_get_current_buf())
        assert.are.equal("", vim.v.warningmsg)
        assert.are.equal("", vim.api.nvim_exec2("messages", { output = true }).output)
        assert.is_true(vim.bo.readonly)
        assert.is_false(vim.bo.modified)
    end)
